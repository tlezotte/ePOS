...
$tableDestination = "Vendor";										// Destination (remote) table
$tableSource = "VEND";												// Source (Local) table
...
switch ($_GET['t']) {
	...
	case 'full':
		...
		while(odbc_fetch_row($rs)) {
			...
			for($i=1;$i<=odbc_num_fields($rs);$i++) { 
			   ...
			   $vendor_data .= "'".htmlentities($field, ENT_QUOTES, 'UTF-8')."',";				// Prepping SQL data
			 }
			$vendor_data2 = preg_replace("/\,$/", "", $vendor_data);							// Remove last comma from SQL data
			$vendor_sql = "INSERT INTO ".$tableDestination." VALUES (" . $vendor_data2 . ")";
	...
	case 'hourly':
		...
		$sql="SELECT * FROM company.".$tableSource." WHERE BTCDAT='".$today."'";				// Get all rows from VEND
		$rs=odbc_exec($conn,$sql);
		if (!$rs) { exit(0); }																	// Connection Failed

		while(odbc_fetch_row($rs)) {
			$btvend = trim(odbc_result($rs,27));
			
			$vendor=$dbh_standards->getRow("SELECT * FROM ".$tableDestination." WHERE BTVEND='".$btvend."'");
			if (!isset($vendor)) {
				$count++;
				$vendor_data = "";																// Reset variable
				for($i=1;$i<=odbc_num_fields($rs);$i++) { 
				...
				$vendor_sql = "INSERT INTO ".$tableDestination." VALUES (" . $vendor_data2 . ")";
				$res = $dbh_standards->query($vendor_sql);
				if (PEAR::isError($res)) { echo "Insert Record (Hourly): " . $res->getMessage() . "<br>"; }
