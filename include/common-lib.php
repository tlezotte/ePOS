 ...
 */
include_once('/var/www/Common/PHP/common-lib.php');

/**
 * - Load Email Functions
 */
include_once('email-lib.php');
...
/**
 * ================= Start Purchasing class =================
 */ 
class Purchasing extends Request {
    ...
    function vendorKickoff(){
		...
		sendCompleted($_POST['req'],$_POST['id'],htmlspecialchars($_POST['purpose']),$_POST['po'], $_SESSION['eid']);		
		//sendVendor($_POST['id']);
    ...
    function update($changeVendor=false){
		if ($changeVendor) {
		...
		debug_capture($_SESSION['eid'], $_POST['id'], 'update', $_SERVER['PHP_SELF'], addslashes(htmlentities($sql)));	
...
/**
 * ================= End Purchasing class =================
 */
 
 
/**
 * ================= Start Tracking class =================
 */
class Tracking extends Request {
	...
	function add(){
		...
		$sql = "INSERT INTO TrackShipments (track_id, type_id, track_number)
							  VALUES (NULL, 
									  '" . mysql_real_escape_string($_POST['type_id']) . "', 
									  '" . mysql_real_escape_string($_POST['track_number']) . "'
									  )";	
		$dbh->query($sql);
	...
	/* ========== Update Tracking information (OFF) ========== */
    function update(){
		global $dbh;
		...
		for ($i=1; $i <= $count; $i++) {
			...
			$track_remove = "track_remove" . $i;
			$track_status = ($_POST[$track_remove] == 'yes') ? 1 : 0;
			
			$sql = "UPDATE TrackShipments SET track_number = '" . mysql_real_escape_string($_POST[$track_number]) . "',
											  track_status = '" . $track_status . "'
									WHERE track_id = " . mysql_real_escape_string($_POST[$track_id]);	
			$dbh->query($sql); 
		}
...
class Status extends Request {
	...
	function set($status) {
		...
		$status_sql="UPDATE PO SET status='". $status ."' WHERE id=" . $_POST['type_id'];
...
class Mysql extends Request {
	/* ========== Record access fro debuging ========== */
	function capture($sql, $action_type='debug') {
		global $dbh;
		
		$sql = "INSERT INTO History (id, type_id, ts, eid, action, page, sql) 
		                     VALUES (NULL, '" . $_POST['type_id'] . "', NOW(), '" . $_SESSION['eid'] . "', '" . $action_type . "', '".$_SERVER['PHP_SELF']."', '" . addslashes(htmlentities($sql)) . "')";
		$dbh->query($sql);
	}
}
/**
