...
if ($_POST['stage'] == 'send') {
	...
	case 'requester':
		...
		break;	
	case 'purchasing':
		$GROUP = "AND U.role = 'purchasing'";
		break;	
	case 'controller':
		$GROUP = "AND U.role = 'controller'";
		break;					
	case 'approver':
	...
	if ($_POST['test'] == '1') {
		sendNotify($default['debug_email'], $_POST['message']);
	} else {
		...
		/* ---------- Get Request System users by group ---------- */
		...
		while($users_sth->fetchInto($USER)) {
			if (!is_null($USER[email])) {
				sendNotify("$USER[email]", $_POST['message']);
			}
		}
...
<!-- #EndLibraryItem --></td>
        ...
        <td valign="top"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                ...
                <td><table border="0" cellpadding="0" cellspacing="0">
                  ...
                  <tr>
                    <td class="BGAccentVeryDarkBorder"><?php
						...
						$form1->addElement('textarea', 'message', 'Message:', array('rows' => 20, 'cols' => 70));
          ...
          <tr>
			...
			*/
			if ($_GET['l'] == 'all') {
				$messages_sq1 = $dbh->prepare("SELECT * FROM Message WHERE type='email' ORDER BY posted DESC");
			?>
            <?php
