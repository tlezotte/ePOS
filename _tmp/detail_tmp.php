...
/* ------------- Give an error when no Requisition number is provided ------------- */
/*if (!array_key_exists('id', $_GET) OR !array_key_exists('id', $_POST)) {
	$_SESSION['error'] = "Requisition Number was not provided.";
	$forward = "../error.php";
...
if (array_key_exists('approval', $_GET)) {
	if ($_GET['approval'] == $AUTH['level'] AND $AUTH[$_GET['approval']] == $_SESSION['eid']) {
		$_GET['approval'] = $_GET['approval'];
	} elseif ($_GET['approval'] != $AUTH['level']) {
		$message="Requisition is currently not at your approval level.";
		unset($_GET['approval']);
	} elseif ($PO['status'] != 'N') {
		switch ($PO['status']) {
			case 'C': $message="Requisition has been marked Canceled."; break;
			case 'X': $message="Requisition has been marked Not Approved."; break;
			case 'A': $message="Requisition has been already Approved."; break;
			case 'O': $message="Requisition has been Kicked off to the Vendor."; break;
		}
		unset($_GET['approval']);	
	}
}
...
<!-- #EndLibraryItem --><!-- InstanceEndEditable --></td>
	...
	<?php 
	  if ($_SESSION['request_access'] == '3') {
	    ...
	    include('../Administration/include/detail_tmp.php');
        ...
        <tr>
          <td><table cellspacing="0" cellpadding="0" width="100%" summary="" border="0">
              <tbody>
                <tr>
                  <td align="center">
				  <div id="hotMessage"  style="width: 700px;display:<?= ($PO['hot'] == 'yes') ? display : none; ?>" onMouseOver="new Effect.Pulsate(this);">This Requisition has been tagged HOT!!</div>	
				  <div id="messageCenter" style="width: 700px;display:<?= (isset($message)) ? display : none; ?>"><?= $message; ?></div>	
                                          ...
                                          <td colspan="6" bgcolor="#<?= $row_color; ?>" class="label"><table border="0">
                                            <tr>
                                              ...
                                              <tr>
                                                ...
                                                <td>Change Category: </td>
                                                <td class="label">&nbsp;</td>
                                                <td nowrap class="label"><?= $PLANT['conbr']; ?>-<?= $PO['department']; ?>-<input name="item<?= $count_items; ?>_newCOAid2" type="text" class="BGAccentDarkBlueBorder" id="item<?= $count_items; ?>_newCOAid2" size="7" maxlength="7" /></td>
