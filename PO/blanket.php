...
if (array_key_exists('l', $_GET)) {
	$WHERE = "WHERE plant=" . $_GET['l'];
}

/* SQL statement to get payments */
$sql = "SELECT cc.id, cc.type_id, p.po, cc.card_id, c.account, cc.amount, DATE_FORMAT(cc.recorded,'%M %e, %Y') AS recorded, CONCAT(e.fst, ' ', e.lst) AS fullname, l.name AS plant, c.controller, c.credit
		FROM CreditCard cc
		  INNER JOIN CreditConfig c ON c.id=cc.card_id
		  INNER JOIN PO p ON p.id=cc.type_id
		  INNER JOIN Standards.Plants l ON l.id=c.plant
		  INNER JOIN Standards.Employees e ON e.eid=cc.eid
		$WHERE";
$query = $dbh->prepare($sql);

/* This weeks execution */
$data_sth = $dbh->execute($query);
$data_count = $data_sth->numRows();	
          ...
          <tr>
            <td class="BGAccentVeryDark"><div align="left">
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                  <tr>
                    <td width="50%" height="30" class="DarkHeaderSubSub">&nbsp;&nbsp;Blanket  Transactions...</td>
            ...
            <td class="BGAccentVeryDarkBorder"><table  border="0" cellspacing="0" cellpadding="0">
                <tr>
                  <td class="BGAccentDarkBorder"><table width="100%"  border="0">
                      <tr class="BGAccentDark" height="25">
                      ...
                      <tr <?php pointer($row_color); ?>>
                        <td bgcolor="#<?= $row_color; ?>" class="padding"><?= $DATA['recorded']; ?></td>
                        <td bgcolor="#<?= $row_color; ?>" class="padding">**** **** **** <?= $DATA['account']; ?></td>
                        <td bgcolor="#<?= $row_color; ?>" class="padding"><?= $DATA['plant']; ?></td>
                        <td bgcolor="#<?= $row_color; ?>" class="padding"><?= $DATA['fullname']; ?></td>
                        <td bgcolor="#<?= $row_color; ?>" class="padding">&nbsp;</td>
                        <td bgcolor="#<?= $row_color; ?>" class="padding"><table width="100%" border="0" cellspacing="0" cellpadding="0">
                          <tr>
                            <td width="10">$</td>
